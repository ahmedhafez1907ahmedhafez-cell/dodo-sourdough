...
// سيرفرات قديمة/غلط اتجربت قبل كده ومبتشتغلش. لو المتغير في البيئة
// لسه بيشاور على واحد منهم (مثلاً قيمة قديمة على Vercel اتنست)،
// بنتجاهله ونستخدم سيرفر التكامل الصح بدل ما الشحنات تفشل بصمت.
const BAD_HOSTS = [
  "test.egypt.api.mylerz.com",   // سيرفر تجريبي قديم — بيرجع 404
  "api.mylerz.net",              // سيرفر البوابة — بيرفض إنشاء الشحنات
  "mylerz.net",
];

function baseUrl() {
  const raw = (process.env.MYLERZ_API_BASE_URL || "").trim()
    .replace(/\/+$/, "").replace(/\/api$/i, "");
  if (!raw) return DEFAULT_BASE;

  let host = "";
  try { host = new URL(raw).hostname.toLowerCase(); } catch { return DEFAULT_BASE; }

  if (BAD_HOSTS.includes(host)) {
    console.warn(
      `[Mylerz] MYLERZ_API_BASE_URL بيشاور على سيرفر مش شغال (${host}) — ` +
      `بنستخدم ${DEFAULT_BASE} بدله. صلّح المتغير في إعدادات Vercel.`
    );
    return DEFAULT_BASE;
  }
  return raw;
...
const SERVICE_NEXT_DAY = "ND";         // اليوم التالي
const SERVICE_SAME_DAY = "SD";         // نفس اليوم
...
export function buildOrderDto(order) {
  ...
  return {
    ...
    Service: order.sameDay ? SERVICE_SAME_DAY : SERVICE_NEXT_DAY,
    Service_Category: SERVICE_CAT_DELIVERY,

    // العميل بيدفع نص المبلغ عربون قبل الشحن، والنص التاني بيحوّله
    // بعد الاستلام — يعني المندوب مبياخدش فلوس خالص. عشان كده
    // الشحنة بتتبعت Pre-Paid والتحصيل صفر.
    Payment_Type: PAYMENT_PREPAID,
    COD_Value: 0,
    Currency: "EGP",
...
// ---- بوليصة الشحن (AWB) ----
// مايلرز بيطلبوا تطبع البوليصة وتلزقها على الطرد — فيها الباركود
// والعنوان، والمندوب بيمسحه. الـ API بيرجّع الـ PDF كنص base64.
//
// بيرجع Buffer فيه الـ PDF، أو بيرمي Error برسالة مفهومة.
export async function getMylerzAwbPdf(trackingNo) {
  if (!isMylerzEnabled()) throw new Error("تكامل مايلرز مش مفعّل");
  const token = await getToken();

  const res = await fetch(`${baseUrl()}/api/packages/GetAWB`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
    body: JSON.stringify({ Barcode: String(trackingNo), ReferenceNumber: "" }),
  });
  ...
  const text = await res.text();
  if (!res.ok) throw new Error(`GetAWB failed (${res.status}): ${text.slice(0, 200)}`);

  let data;
  try { data = JSON.parse(text); } catch { throw new Error("رد مايلرز مش JSON مفهوم"); }
  if (data.IsErrorState) throw new Error(data.ErrorDescription || "مايلرز رفضت طلب البوليصة");
  if (!data.Value) throw new Error("مايلرز رجّعت بوليصة فاضية");
