...
## 7. مايلرز Mylerz (شركة الشحن)
التكامل مكتوب في `lib/mylerz.js` على أساس توثيق **Mylerz API V1.3** الرسمي.
أي أوردر جديد (بنها أو أي محافظة) بيتبعت لمايلرز أوتوماتيك كشحنة
**دفع عند الاستلام (COD)** بقيمة الأوردر كله، ورقم التتبع بيتسجل على
الأوردر وبيظهر في لوحة الأدمن.

### خطوات التفعيل
1. رابط الـ API (مش رابط البوابة اللي بتفتحها في المتصفح):
   - **الإنتاج (حساب dodo_sourdough الحقيقي):** `https://api.mylerz.net`
   - **التجربة (حساب dodo_sourdough_test اللي عملناه على test.egypt.mylerz.com):** `https://test.egypt.api.mylerz.com`
   - ابدأ بالتجربة لحد ما تتأكد كل حاجة شغالة، وبعدين بدّل لرابط الإنتاج
2. حط اليوزر والباسورد بتوع الحساب اللي هتستخدمه في `MYLERZ_USERNAME` و `MYLERZ_PASSWORD`
3. اسأل مايلرز عن اسم الـ Warehouse المسجل لحسابك وحطه في `MYLERZ_WAREHOUSE_NAME`
   (لو سيبته فاضي غالباً هياخدوا الافتراضي)
4. غيّر `MYLERZ_ENABLED=true` واعمل أوردر تجربة من الموقع
5. لما تتأكد إن الشحنة بتوصل لحسابهم التجريبي، بدّل الرابط برابط الإنتاج

### أسعار التوصيل
موجودة في `lib/deliveryRates.js` — متسحوبة من جدول أسعار مايلرز الرسمي
(Delta Rate Sheet)، والعميل بيدفع التكلفة بالظبط شاملة ضريبة 14%:
| المنطقة | قبل الضريبة | اللي العميل بيدفعه |
|---|---|---|
| الدلتا (وبنها) | 75 | 85.5 |
| الإسكندرية | 80 | 91.2 |
| القاهرة والجيزة | 85 | 96.9 |
| مدن القناة | 85 | 96.9 |
| الصعيد والبحر الأحمر ومطروح | 95 | 108.3 |
| سيناء والوادي الجديد | 140 | 159.6 |

لو مايلرز غيّروا الأسعار، عدّل أرقام `MYLERZ_ZONES` في `lib/deliveryRates.js` بس.
ملحوظة: الأسعار دي لأول 2 كيلو — لو ضفت منتجات تقيلة بعدين هنحتاج نضيف وزن لكل منتج.
