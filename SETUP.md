...
## 7. بوسطة Bosta (شركة الشحن)
التكامل مكتوب في `lib/bosta.js` على أساس الـ API الرسمي لبوسطة
(`https://app.bosta.co/api/v0`) و الـ SDK بتاعهم على GitHub.

الأوردر **مش** بيتشحن أوتوماتيك أول ما يتسجّل. بيتشحن بس لما تدوس
«تم استلام العربون» في لوحة الأدمن — يعني بعد ما تشوف صورة التحويل
بعينك. الشحنة بتتبعت **بتحصيل صفر (COD = 0)** لأن العميل بيدفع ٥٠٪
عربون قبل الشحن والباقي تحويل بعد الاستلام، فالمندوب مبياخدش فلوس.

### خطوات التفعيل
1. اعمل حساب على https://business.bosta.co/signup (مش محتاج سجل تجاري)
2. الإعدادات ← **ربط التطبيقات** ← اطلب OTP ← **إنشاء مفتاح API**
3. حط المفتاح في `.env.local` في `BOSTA_API_KEY` — **بإيدك**،
   ومتبعتوش في أي شات أو رسالة
4. سيب `BOSTA_API_BASE_URL=https://app.bosta.co` للإنتاج،
   أو `https://stg-app.bosta.co` لو عايز تجرب على بيئة التجربة
5. `BOSTA_SKIP_BANHA=true` معناها إن أوردرات بنها متروحش لبوسطة
   لأنك بتوصّلها بنفسك
6. اعمل أوردر تجربة وأكّد عربونه، وشوف رقم التتبع ظهر في الأدمن

### تحديثات حالة الشحنة (Webhook)
عشان الأدمن يتحدّث لوحده لما الشحنة توصل:
1. الإعدادات ← ربط التطبيقات ← **إضافة رابط الـ Webhook**
2. رابط Webhook: `https://dodo-sourdough.vercel.app/api/webhooks/bosta`
3. اسم مفتاح التوثيق: نفس قيمة `BOSTA_WEBHOOK_HEADER`
4. مفتاح التوثيق: نفس قيمة `BOSTA_WEBHOOK_SECRET`

من غير السر ده الـ endpoint بيرفض أي طلب — مهم، لأن اللينك مفتوح
على النت وأي حد يقدر يناديه.

### الأوردرات القديمة
لو عندك أوردرات اتشحنت بشركة الشحن القديمة، شغّل مرة واحدة بس:
`npm run migrate:shipment`
بينقل حقول الشحن القديمة للأسماء الجديدة عشان تفضل بايناها في الأدمن.

### أسعار التوصيل
موجودة في `lib/deliveryRates.js`. العميل بيدفع التكلفة شاملة ضريبة 14%:

| المنطقة | قبل الضريبة | اللي العميل بيدفعه |
|---|---|---|
| القاهرة والجيزة | 75 | 85.5 |
| الإسكندرية | 80 | 91.2 |
| الدلتا | 85 | 96.9 |
| مدن القناة | 90 | 102.6 |
| الصعيد والبحر الأحمر | 105 | 119.7 |
| مناطق بعيدة | 150 | 171 |

⚠️ بنها مالهاش سعر ثابت — بيتحدد يدوي على واتساب.
